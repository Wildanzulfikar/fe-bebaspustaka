...
function Loader() {
  return (
    <div className="flex justify-center items-center w-full py-10">
      <div className="w-8 h-8 border-4 border-[#008797] border-t-transparent rounded-full animate-spin"></div>
    </div>
  );
}
...
export default function MainBebasPustaka() {
  ...
  useEffect(() => {
    ...
    if (jurusan !== "") {
      temp = temp.filter((item) => item.prodi === jurusan);
    }

    if (statusPustaka !== "") {
      temp = temp.filter((item) => item.status === statusPustaka);
    }

    if (statusPinjaman !== "") {
      temp = temp.filter((item) => {
        if (statusPinjaman === "Belum") {
          return item.status_pinjaman === "" || item.status_pinjaman === null;
        }
      ...
      <div className="flex flex-col w-full gap-4">
        ...
        <div ref={wrapperRef} className="flex flex-wrap items-center gap-4 px-4">
          ...
          <div className="relative w-64">
            ...
            {openStatusPustaka &&
              renderDropdown(statusPustakaOptions, setStatusPustaka, setOpenStatusPustaka, "statusPustaka")}
          </div>
          ...
          <div className="relative w-64">
            ...
            {openStatusPinjaman &&
              renderDropdown(statusPinjamanOptions, setStatusPinjaman, setOpenStatusPinjaman, "statusPinjaman")}
          </div>
        </div>
